...
interface AdmissionLetterPrintModalProps {
  serialNumber: string;
...
export default function AdmissionLetterPrintModal({
  serialNumber,
  applicantName,
  email,
  phone,
  ...
  isOpen,
  onClose,
}: AdmissionLetterPrintModalProps) {
  const [qrUrl, setQrUrl] = useState<string>('');

  const serialSafe = serialNumber.replace(/\//g, '_');
  const verifyUrl = `${typeof window !== 'undefined' ? window.location.origin : ''}/verify/${serialSafe}`;
  ...
  if (nameLower.includes('3 months') || ['CAP', 'CSPV', 'CMR'].includes(codeUpper)) {
    ...
    durationStr = '2 Years (Certificate Level)';
  }
  ...
  const reportingDateStr = new Date(reportingDate).toLocaleDateString('en-KE', {
    day: 'numeric',
    month: 'long',
    year: 'numeric',
  });
  ...
  return (
    <div
      className="admission-print-overlay no-print-backdrop"
      style={{
        position: 'fixed',
        inset: 0,
    ...
    >
      <div
        className="admission-print-wrapper"
        style={{
          width: '100%',
          maxWidth: '850px',
          maxHeight: '92vh',
      ...
      >
        ...
        <div
          ...
          style={{
            ...
            color: '#ffffff',
            borderBottom: '2px solid var(--accent-gold, #b7791f)',
          }}
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <span style={{ fontSize: '18px' }}>🎓</span>
            <div>
              <strong style={{ fontSize: '14px', letterSpacing: '0.5px' }}>Official Admission Letter Print Preview</strong>
              <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.7)' }}>Ref: {serialNumber}</div>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            <button
              ...
              style={{
                ...
                fontSize: '13px',
                fontWeight: 700,
                background: 'var(--accent-gold, #b7791f)',
                color: '#ffffff',
                borderRadius: '4px',
                border: 'none',
                cursor: 'pointer',
              }}
              id="print-letter-trigger-btn"
            >
              <Printer size={16} /> Print Admission Letter
            </button>
            <a
              href={`/api/letters/download/${serialNumber}`}
              download
        ...
        >
          {/* Printable Sheet (Simulates A4 210mm) */}
          <div
          ...
          >
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '2px solid #14213d', paddingBottom: '16px', marginBottom: '18px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                ...
                <div>
                  <h1 style={{ fontSize: '20px', fontWeight: '800', color: '#14213d', margin: 0, letterSpacing: '0.5px', textTransform: 'uppercase' }}>
              ...
              {qrUrl && (
                <div style={{ textAlign: 'center', padding: '4px', border: '1px solid #cbd5e1', borderRadius: '4px', background: '#f8fafc' }}>
                  ...
                  <img src={qrUrl} alt="Verify QR Code" style={{ width: '68px', height: '68px', display: 'block' }} />
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '12px', fontFamily: 'Arial, sans-serif', fontWeight: 'bold', marginBottom: '16px', color: '#334155' }}>
              <div>
                ...
                <span style={{ color: '#14213d', padding: '2px 6px', background: '#f1f5f9', borderRadius: '3px' }}>{serialNumber}</span>
              </div>
              <div>
            ...
            <div style={{ marginBottom: '18px', padding: '12px 16px', background: '#f8fafc', borderLeft: '4px solid #14213d', borderRadius: '0 4px 4px 0', fontSize: '12.5px', fontFamily: 'Arial, sans-serif' }}>
              ...
              <div style={{ fontSize: '15px', fontWeight: '800', color: '#0f172a', textTransform: 'uppercase' }}>{applicantName}</div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '4px 20px', marginTop: '6px', color: '#334155' }}>
                <div><strong>Email:</strong> {email}</div>
                <div><strong>Phone:</strong> {phone}</div>
                <div><strong>KCSE Index No:</strong> {kcseIndexNo}</div>
                <div><strong>Mean Grade:</strong> {kcseMeanGrade}</div>
              </div>
            </div>
            ...
            <div style={{ textAlign: 'center', margin: '20px 0 16px', borderBottom: '1.5px solid #0f172a', paddingBottom: '6px' }}>
              <h2 style={{ fontSize: '16px', fontWeight: '800', textTransform: 'uppercase', color: '#0f172a', letterSpacing: '0.5px', margin: 0 }}>
                RE: PROVISIONAL ADMISSION LETTER ({programmeCode})
              </h2>
            </div>
            ...
            <div style={{ fontSize: '13.5px', color: '#1e293b', marginBottom: '16px', textAlign: 'justify' }}>
              ...
              <div style={{ margin: '12px 0', padding: '12px 18px', background: '#f1f5f9', border: '1px solid #cbd5e1', borderRadius: '4px', fontFamily: 'Arial, sans-serif' }}>
                <div style={{ display: 'grid', gridTemplateColumns: '140px 1fr', gap: '6px', fontSize: '12.5px' }}>
                  ...
                  <strong style={{ color: '#14213d' }}>{programmeName}</strong>
                  
                  <span style={{ fontWeight: 'bold', color: '#475569' }}>Programme Code:</span>
                  <span>{programmeCode}</span>
                ...
                <tbody>
                  ...
                  <tr style={{ background: '#f1f5f9', fontWeight: 'bold' }}>
                    ...
                    <td style={{ padding: '8px 10px', border: '1px solid #cbd5e1', textAlign: 'right', color: '#14213d', fontSize: '13px' }}>
                      KES {feesSummary.reduce((acc, curr) => acc + curr.total, 0).toLocaleString()}
                    </td>
